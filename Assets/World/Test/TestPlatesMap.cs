...
namespace World.Test
{
    public class TestPlatesMap : MonoBehaviour
    {
        ...
        public void Save()
        {
            ...
            {
                max = Mathf.Max(max, heightMapRes[i]);
            }
            var heightTexRes = new Texture2D(heightMap.width, heightMap.height, TextureFormat.R16, false);


            for (int i = 0; i < heightMapRes.Length; i++)
            {
                heightTexData[i] = new Color(heightMapRes[i] / max, heightMapRes[i] / max, heightMapRes[i] / max);
            }
